...
/**
 * HikariCP DataSource 팩토리. 환경변수 기반으로 write(Source) / read(Replica) 커넥션 풀을 생성한다.
 */
public class HikariDataSourceFactory {
	...
	/**
	 * Write(Source) 용 DataSource 생성. DB_SOURCE_* 환경변수를 사용한다.
	 */
	public static HikariDataSource createWriteDataSource() {
		...
		config.setJdbcUrl(
				buildJdbcUrl(env("DB_SOURCE_HOST", "mysql-router"), env("DB_SOURCE_PORT", "6446"), dbName, dbParams));
	...
	/**
	 * Read(Replica) 용 DataSource 생성. DB_REPLICA_* 환경변수를 사용하며, 미설정 시 Source 값으로
	 * fallback.
	 */
	public static HikariDataSource createReadDataSource() {
		...
		config.setJdbcUrl(buildJdbcUrl(env("DB_REPLICA_HOST", env("DB_SOURCE_HOST", "mysql-router")),
				env("DB_REPLICA_PORT", env("DB_SOURCE_PORT", "6447")), dbName, dbParams));
