...
public class UserDAO {
	...
	public UserDAO(@Qualifier("writeDataSource")  DataSource dataSource) {
	...
	public UserDTO findById(String id) throws SQLException {
		...
		try (Connection conn = writeDataSource.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {

			pstmt.setString(1, id);

			try (ResultSet rs = pstmt.executeQuery()) {
				if (rs.next()) {
					return UserDTO.builder().id(rs.getString("ID")).password(rs.getString("PASSWORD"))
