# <img width="30" height="30" alt="image" src="https://github.com/user-attachments/assets/fb3143f7-f083-49ce-b795-319c67cce409" />  Woori Card Scope
...
| `total` | 후보 서버들의 `effective_weight` 합산값. 선택된 서버의 `current_weight`에서 차감하는 데 사용 |
...
```
---
### current_weight 기반 가중 라운드로빈 예시

WAS4, WAS5 동률(부하율 0.5) → `current_weight`로 타이브레이킹

- 두 서버 모두 weight = 5, total = 10

| 시점 | WAS4 current_weight | WAS5 current_weight | 선택 | 선택 후 차감 |
|------|---------------------|---------------------|------|-------------|
| 1회차 | 5 | 5 | WAS4 (먼저 발견) | WAS4: 5-total = **-5** |
| 2회차 | -5+5=0 | 5+5=10 | WAS5 (더 큼) | WAS5: 10-total = **0** |
| 3회차 | 0+5=5 | 0+5=5 | WAS4 | WAS4: 5-total = **-5** |

> 선택된 서버는 `current_weight -= total`로 차감되므로, 다음 라운드에서 상대적으로 낮아지고 **못 받은 서버가 자연스럽게 선택**된다.
...
https://github.com/user-attachments/assets/abedeaf3-a3a5-4daa-b443-ad3d727afed7
